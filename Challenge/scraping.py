...
def hemisphere_data(browser):
    ...
    #     a) click on each hemisphere link
        ...
        hemisphere_data = hemisphere_scrape(browser.html)
...
def hemisphere_scrape(browser):
    ...
    try:
        ...
        # Use the parent element to find the first 'a' tag and save it as 'news_title'
        hemisphere_title = slide_elem.find('h2', class_='title').get_text()
        # Use the parent element to find the paragraph text
        hemisphere_sample = slide_elem.find('a', text='Sample').get_text('href')

    except AttributeError:
        return None, None

    hemisphere = {"title" : hemisphere_title,
                "hemisphere_url" : hemisphere_sample}
